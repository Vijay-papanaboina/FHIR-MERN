...
const FHIR_BASE = env.FHIR_BASE_URL;
const PATIENT_COUNT = 5;

const FHIR_SECRET = env.FHIR_SECRET;
...
function generatePatientBundle(index: number): FhirBundle {
  ...
  if (!p) {
    throw new Error(
      `Invalid patient index: ${index}. Max is ${patients.length - 1}`,
    );
  }
  ...
  for (let month = 5; month >= 0; month--) {
    ...
    for (const vt of vitalTypes) {
      const value = +(
        vt.range[0]! +
        Math.random() * (vt.range[1]! - vt.range[0]!)
      ).toFixed(1);
...
async function postBundle(bundle: FhirBundle): Promise<void> {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `FHIR server returned ${res.status}: ${text.slice(0, 200)}`,
    );
  }
}
...
async function main() {
  ...
  console.log(
    `   Generating ${PATIENT_COUNT} patients with 6 months of vitals...\n`,
  );
  ...
  console.log(
    `   Search for "Smith", "Garcia", "Johnson", "Williams", or "Chen" in the app.`,
  );
}
