...
describe("user validators", () => {
  it("linkPatientSchema accepts valid fhirPatientId", () => {
    const parsed = linkPatientSchema.safeParse({ fhirPatientId: "Patient-1001" });
  ...
  it("linkPatientSchema rejects empty and whitespace-only fhirPatientId", () => {
    expect(linkPatientSchema.safeParse({ fhirPatientId: "" }).success).toBe(false);
  ...
  it("linkPatientSchema rejects overly long and injection-like input", () => {
    ...
    expect(linkPatientSchema.safeParse({ fhirPatientId: tooLong }).success).toBe(
      false,
    );
    expect(
