...
import { getPatientVitals, createPatientVital } from '../services/vitals.service.js';
...
const createVitalSchema = z.object({
    ...
    display: z.string().min(1, 'Display name is required').max(256, 'Display name is too long'),
    value: z.number({ message: 'Value is required' }).finite('Value must be a finite number'),
    unit: z.string().min(1, 'Unit is required'),
    unitCode: z.string().min(1, 'Unit code is required'),
    effectiveDateTime: z.string().datetime({ message: 'Must be a valid ISO datetime' }).optional(),
});
...
export const createVitalHandler = async (req: Request, res: Response) => {
    ...
    if (!idResult.success) {
        throw new AppError(idResult.error.issues[0]?.message ?? 'Invalid Patient ID', 400);
    }

    const bodyResult = createVitalSchema.safeParse(req.body);
    if (!bodyResult.success) {
        throw new AppError(bodyResult.error.issues[0]?.message ?? 'Invalid vital data', 400);
    }
