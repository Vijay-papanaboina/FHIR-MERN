 ...
 */
export const getFhirHeaders = (includeContentType = false): Record<string, string> => {
 ...
 */
const fhirFetch = async (url: string, options: FhirFetchOptions = {}): Promise<Record<string, unknown>> => {
    ...
    try {
        ...
        if (!response.ok) {
            throw new AppError(`FHIR ${method} failed with status ${response.status}`, 502);
        }
