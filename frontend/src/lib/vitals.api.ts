...
export function createVital(patientId: string, input: CreateVitalInput): Promise<VitalsDTO> {
    const trimmed = patientId.trim()
    if (!trimmed) return Promise.reject(new Error("Patient ID is required"))
    return apiPost<VitalsDTO>(`/api/patients/${encodeURIComponent(trimmed)}/vitals`, input)
}
